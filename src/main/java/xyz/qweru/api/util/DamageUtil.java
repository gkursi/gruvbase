...
public class DamageUtil {
    ...
    private static float explosionDamage(LivingEntity target, Vec3d explosionPos, float power, boolean predictMovement, RaycastFactory raycastFactory) {
        ...
        if (target instanceof PlayerEntity player && EntityUtil.getGamemode(player) == GameMode.CREATIVE) return 0f;
    ...
    private static float modifyAttackDamage(LivingEntity attacker, LivingEntity target, ItemStack weapon, DamageSource damageSource, float damage) {
        ...
        InventoryUtil.getEnchantments(weapon, enchantments);
        float enchantDamage = 0f;

        int sharpness = InventoryUtil.getEnchantmentLevel(enchantments, Enchantments.SHARPNESS);
        if (sharpness > 0) {
            enchantDamage += 1 + 0.5f * (sharpness - 1);
        }

        int baneOfArthropods = InventoryUtil.getEnchantmentLevel(enchantments, Enchantments.BANE_OF_ARTHROPODS);
        ...
        int impaling = InventoryUtil.getEnchantmentLevel(enchantments, Enchantments.IMPALING);
        if (impaling > 0 && target.getType().isIn(EntityTypeTags.SENSITIVE_TO_IMPALING)) {
            enchantDamage += 2.5f * impaling;
        }

        int smite = InventoryUtil.getEnchantmentLevel(enchantments, Enchantments.SMITE);
        ...
        if (attacker instanceof PlayerEntity playerEntity) {
            ...
            if (weapon.getItem() instanceof MaceItem item) {
                ...
                if (bonusDamage > 0f) {
                    int density = InventoryUtil.getEnchantmentLevel(weapon, Enchantments.DENSITY);
    ...
    public static float calculateReductions(float damage, LivingEntity entity, DamageSource damageSource) {
        ...
        // Armor reduction
        damage = net.minecraft.entity.DamageUtil.getDamageLeft(entity, damage, damageSource, getArmor(entity), (float) entity.getAttributeValue(EntityAttributes.ARMOR_TOUGHNESS));
    ...
    private static float protectionReduction(LivingEntity player, float damage, DamageSource source) {
        ...
        for (ItemStack stack : player.getAllArmorItems()) {
            Object2IntMap<RegistryEntry<Enchantment>> enchantments = new Object2IntOpenHashMap<>();
            InventoryUtil.getEnchantments(stack, enchantments);

            int protection = InventoryUtil.getEnchantmentLevel(enchantments, Enchantments.PROTECTION);
            if (protection > 0) {
                damageProtection += protection;
            }

            int fireProtection = InventoryUtil.getEnchantmentLevel(enchantments, Enchantments.FIRE_PROTECTION);
            if (fireProtection > 0 && source.isIn(DamageTypeTags.IS_FIRE)) {
                damageProtection += 2 * fireProtection;
            }

            int blastProtection = InventoryUtil.getEnchantmentLevel(enchantments, Enchantments.BLAST_PROTECTION);
            if (blastProtection > 0 && source.isIn(DamageTypeTags.IS_EXPLOSION)) {
                damageProtection += 2 * blastProtection;
            }

            int projectileProtection = InventoryUtil.getEnchantmentLevel(enchantments, Enchantments.PROJECTILE_PROTECTION);
            if (projectileProtection > 0 && source.isIn(DamageTypeTags.IS_PROJECTILE)) {
                damageProtection += 2 * projectileProtection;
            }

            int featherFalling = InventoryUtil.getEnchantmentLevel(enchantments, Enchantments.FEATHER_FALLING);
