...
public class FontRenderer {
     ...
     * @param offset font offset
     */
    public FontRenderer(Object font, Identifier fontPath, float size, float offset) {
        this.fontSize = size;
        this.offset = offset;
        this.startChar = 32;
        this.endChar = 255;
    ...
    private void createFont(Object font, float size, Identifier fontPath) {
        try {
            if (font instanceof Font)
                ...
                if(font == "Verdana") this.theFont = new Font("Verdana", Font.PLAIN, Math.round(size));
                else this.theFont = Font.createFont(Font.TRUETYPE_FONT, Objects.requireNonNull(FontRenderer.class.getResourceAsStream("/assets/pulse/" + fontPath.getPath() + "/" + font))).deriveFont(size);
            }
            this.theGraphics.setFont(this.theFont);
        } catch (Exception e) {
            Gruvhack.LOGGER.error("Error loading font: {}", font.toString());
    ...
    public void drawOrderedText(MatrixStack matrixStack, OrderedText text, double x, double y) {
        ColoredString string = ColoredString.of("", Color.GREEN, this);
        ...
        this.drawColoredString(matrixStack, string, (float) x, (float) (y));
    }
     ...
     * @author qweru
     */
    public record ColoredString(ArrayList<ColoredStringSegment> segments, FontRenderer font) {

        public static ColoredString of(String str, Color color, FontRenderer font) {
            return new ColoredString(new ArrayList<>(Collections.singleton(new ColoredStringSegment(str, color, font))), font);
        }

        public ColoredString add(String s, Color color) {
            segments.add(new ColoredStringSegment(s, color, font));
            return this;
        }
        ...
        public record ColoredStringSegment(String text, Color color, FontRenderer font) {
            public float width() {
